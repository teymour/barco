...
with open(zeltyfile) as f:
    ...
    for o in zeltydata['orders']:
        for i in o['items']:
            ...
            converted.append([date.strftime('%Y-%m-%d'), o['uuid'], i['item_id'], i['name'].replace(';', ' ').replace(',', ' '), i['price']['final_amount_inc_tax'] / 100])

z = pd.DataFrame(converted, columns=['date', 'order uuid', 'pid', 'produit', 'price'])
